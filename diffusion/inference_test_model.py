...
def main():
    ...
    if DIT_CHECKPOINT is not None:
        abstract_state = {
            "model": jax.tree.map(ocp.utils.to_shape_dtype_struct, nnx.state(dit)),
            "optimizer": jax.tree.map(ocp.utils.to_shape_dtype_struct, nnx.state(optimizer)),
        }
        restored = ocp.StandardCheckpointer().restore(DIT_CHECKPOINT, abstract_state)
    ...
    def generate(dit, vae, noise, num_steps, compression_mask, rngs):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch % 2 == 0:
            ...
            reconstruction, gen_video_mask = generate(
                dit, vae, noise, NUM_SAMPLE_STEPS,
                ref_compression_mask, rngs,
            )
            recon_batch = {
